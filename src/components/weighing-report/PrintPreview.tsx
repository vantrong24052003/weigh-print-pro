...
export const PrintPreview = ({ data, no = '', onClose, onQzPrint }: PrintPreviewProps) => {
  ...
  return (
    <div className='fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4 backdrop-blur-md'>
      <div className='bg-white rounded-3xl shadow-2xl max-w-105 w-full overflow-hidden flex flex-col max-h-[95vh]'>
        <div className='p-4 px-6 border-b border-slate-100 flex justify-between items-center'>
          ...
          <button
            onClick={onClose}
            className='cursor-pointer p-2 hover:bg-slate-50 rounded-full transition-colors text-slate-400'
          >
            <svg
        ...
        <div className='flex-1 overflow-y-auto bg-slate-100 flex flex-col items-center py-6 px-4'>
          <div className='bg-white shadow-xl w-fit h-fit min-h-fit mb-12 relative shrink-0'>
            <div
              style={{
                height: 6,
                background: 'repeating-linear-gradient(90deg,#cbd5e1 0,#cbd5e1 4px,white 4px,white 8px)'
              }}
            />
            <pre
              style={{
